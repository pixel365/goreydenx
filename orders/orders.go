package orders

import (
	"bytes"
	"encoding/json"
	"fmt"

	rx "github.com/pixel365/goreydenx"
	h "github.com/pixel365/goreydenx/helpers"
	m "github.com/pixel365/goreydenx/model"
)

// Orders All orders
// See https://api.reyden-x.com/docs#/Orders/orders_v1_orders__get
func Orders(c rx.RxClient, cursor string) (*m.Result[[]m.Order], error) {
	path := "/orders/"
	if len(cursor) > 0 {
		path = fmt.Sprintf("/orders/?cursor=%s", cursor)
	}
	return h.Get[[]m.Order](c, path)
}

// Details Order details
// See https://api.reyden-x.com/docs#/Orders/order_details_v1_orders__order_id___get
func Details(c rx.RxClient, orderId uint32) (*m.Result[m.Order], error) {
	return h.Get[m.Order](c, fmt.Sprintf("/orders/%d/", orderId))
}

// Payments Order payments
// See https://api.reyden-x.com/docs#/Orders/order_payments_v1_orders__order_id__payments__get
func Payments(c rx.RxClient, orderId uint32, cursor string) (*m.Result[[]m.Payment], error) {
	path := fmt.Sprintf("/orders/%d/payments/", orderId)
	if len(cursor) > 0 {
		path = fmt.Sprintf("/orders/%d/payments/?cursor=%s", orderId, cursor)
	}
	return h.Get[[]m.Payment](c, path)
}

// OnlineStats Detailed information about users online
// See https://api.reyden-x.com/docs#/Orders/order_stats_online_v1_orders__order_id__statistics_online__get
func OnlineStats(c rx.RxClient, orderId uint32) (*m.Result[[]m.OnlineStats], error) {
	return h.Get[[]m.OnlineStats](c, fmt.Sprintf("/orders/%d/statistics/online/", orderId))
}

// ClicksStats Detailed information about clicks
// See https://api.reyden-x.com/docs#/Orders/order_stats_clicks_v1_orders__order_id__statistics_clicks__get
func ClicksStats(c rx.RxClient, orderId uint32) (*m.Result[[]m.DateQuantity], error) {
	return h.Get[[]m.DateQuantity](c, fmt.Sprintf("/orders/%d/statistics/clicks/", orderId))
}

// ViewsStats Detailed information about views
// See https://api.reyden-x.com/docs#/Orders/order_stats_views_v1_orders__order_id__statistics_views__get
func ViewsStats(c rx.RxClient, orderId uint32) (*m.Result[[]m.DateQuantity], error) {
	return h.Get[[]m.DateQuantity](c, fmt.Sprintf("/orders/%d/statistics/views/", orderId))
}

// SitesStats Detailed information about sites
// See https://api.reyden-x.com/docs#/Orders/order_stats_sites_v1_orders__order_id__statistics_sites__get
func SitesStats(c rx.RxClient, orderId uint32) (*m.Result[[]m.SiteStats], error) {
	return h.Get[[]m.SiteStats](c, fmt.Sprintf("/orders/%d/statistics/sites/", orderId))
}

// MultipleViewsStats View statistics for multiple orders
// See https://api.reyden-x.com/docs#/Orders/multiple_views_v1_orders_multiple_views__post
func MultipleViewsStats(
	c rx.RxClient,
	identifiers m.Identifiers,
) (*m.Result[[]m.IdQuantity], error) {
	payload, err := json.Marshal(identifiers)
	if err != nil {
		return nil, err
	}
	return h.Post[[]m.IdQuantity](c, "/orders/multiple/views/", bytes.NewBuffer(payload))
}

// MultipleClicksStats Click-through statistics for multiple orders
// See https://api.reyden-x.com/docs#/Orders/multiple_clicks_v1_orders_multiple_clicks__post
func MultipleClicksStats(
	c rx.RxClient,
	identifiers m.Identifiers,
) (*m.Result[[]m.IdQuantity], error) {
	payload, err := json.Marshal(identifiers)
	if err != nil {
		return nil, err
	}
	return h.Post[[]m.IdQuantity](c, "/orders/multiple/clicks/", bytes.NewBuffer(payload))
}

// CreateStream Create new order for Twitch or Youtube stream
// See https://api.reyden-x.com/docs#/Orders/twitch_stream_v1_orders_create_twitch_stream__post
func CreateStream[T m.OrderParams](c rx.RxClient, params T) (*m.ActionResult, error) {
	valid, err := params.IsValid()
	if !valid {
		return nil, err
	}

	payload, err := json.Marshal(params)
	if err != nil {
		return nil, err
	}

	res, err := c.Post(
		fmt.Sprintf("/orders/create/%s/stream/", params.PlatformCode()),
		bytes.NewBuffer(payload),
	)
	if err != nil {
		return nil, err
	}

	result := m.ActionResult{}
	err = json.Unmarshal(res, &result)
	return &result, err
}
